"""Minimal parity check: plain pi05 vs frozen pi05crl action loss.

Loads the SAME pretrained checkpoint (pi05_libero) into both a plain `Pi0Config`
model and a `Pi0CRLConfig` model, then pushes the *same* batches through
...
pure forward pass). The CRL model's action loss reads only the current-obs half,
...
Expectation: if the shared forward path is equivalent to plain pi05, the
two columns match. A ~10x gap localizes the bug to the Pi0CRL forward.
"""
...
def main():
    ...
    print("Building + loading pi05crl (all weights from same checkpoint) ...")
    ...
    loader = _data_loader.create_data_loader(small_cfg, shuffle=False, num_batches=NUM_BATCHES, skip_norm_stats=False)

    print(f"\n{'batch':>5}  {'plain pi05':>12}  {'pi05crl':>12}  {'ratio':>8}")
    plain_all, rep_all = [], []
    for i, (obs, actions) in enumerate(loader):
        rng_b = jax.random.fold_in(rng, i)

        plain_chunked, _ = plain.compute_loss(rng_b, {"observation": obs, "actions": actions}, train=False)
        plain_loss = jnp.mean(plain_chunked)

        # rep: future_observation = obs (its half is discarded and cannot affect the action loss).
        _, info = rep.compute_loss(
            rng_b, {"observation": obs, "future_observation": obs, "actions": actions}, train=False
        )
    ...
    print(
        ...
        "  ratio ~= 10   -> the Pi0CRL forward is degrading the action loss; inspect embed/positions/masks there.\n"
