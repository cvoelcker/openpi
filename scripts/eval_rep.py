"""Offline rep-checkpoint evaluation: load a trained rep-model checkpoint (Pi0CRL / Pi0SF) and
push goal-conditioned batches through `compute_loss` (train=False), aggregating all diagnostics —
rep_loss, the masked/nomask collision decomposition, and the val ranking probe (rank_order_*,
rank_neg_*, ranking_loss). No training state, no wandb; results print to stdout.
...
def main(args: Args) -> None:
    ...
    def eval_step(model, rng, batch):
        # The model reads the keys it needs (train=False enables the CRL ranking probe when the
        # loader provides the frame-index keys) and ignores the rest.
    ...
    for i in range(args.num_batches):
        ...
        info = eval_step(model, jax.random.fold_in(rng, i), dict(batch))
