...
Usage (run on a GPU box / cluster, not a laptop -- this loads a multi-B backbone):
    ...
    uv run scripts/probe_rep_layers.py pi05_crl_libero_finetune \
...
def _infonce(phi: jnp.ndarray, psi: jnp.ndarray) -> tuple[jnp.ndarray, jnp.ndarray]:
    ...
    Mirrors the contrast in pi05crl.compute_loss (raw dot products, no temperature),
...
def build_feature_fn(graphdef):
    ...
    def feature_fn(state, obs, future):
        ...
        # pi0._run_prefix (adarms_cond defaults to None per expert; return_hidden gives
        # a third output that this probe reads its own features instead of).
        outs, kv_cache, _ = model.PaliGemma.llm([tokens, None], mask=attn_mask, positions=positions)
...
def main():
    ...
    p.add_argument("config_name", nargs="?", help="Registered TrainConfig name (e.g. pi05_crl_libero_finetune).")
